...
<body class="bg-cover bg-center bg-green-200"
    ...
    <div class="container mx-auto mt-8 p-8 bg-white max-w-md">
        <h2 class="text-2xl font-bold mb-4 text-green-700 text-center">Edit recipe</h2>
        <form action="/edit-post/{{$post->id}}" method="post">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="title" class="block text-gray-700 text-sm font-semibold mb-2">Recipe Title</label>
                <input type="text" name="title" id="title" placeholder="Recipe Title" value="{{$post->title}}" class="w-full p-2 border border-green-300 rounded-md">
            </div>
            <div class="mb-4">
                <label for="description" class="block text-gray-700 text-sm font-semibold mb-2">Recipe Description</label>
                <textarea name="body" id="description" placeholder="Recipe Description" rows="4" class="w-full p-2 border border-green-300 rounded-md resize-none">{{$post->body}}</textarea>
            </div>
            <button class="w-full bg-green-900 mb-4 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                Save Changes
            </button>
        </form>
    </div>
